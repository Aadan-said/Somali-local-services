...
export default function ClientNotifications() {
    ...
    const handleNotificationPress = async (notification: Notification) => {
        ...
        const getMobilePath = (link: string | null | undefined, type: string) => {
            if (!link) {
                ...
                if (type.includes('PROVIDER') || type.includes('JOB')) return '/(client)/jobs';
                if (type === 'PAYMENT_REQUEST') return '/(client)/wallet';
                return '/(client)/jobs';
            }
            ...
            if (link.includes('/client/wallet')) return '/(client)/wallet';

            // Default fallbacks for other web paths
            if (link.startsWith('/client')) return '/(client)/jobs';
    ...
    const getIcon = (type: string) => {
        switch (type) {
            case 'PROVIDER_ACCEPTED': return { name: 'check-circle', color: '#10b981' };
            case 'JOB_STARTED': return { name: 'play-circle', color: '#3b82f6' };
            case 'JOB_COMPLETED': return { name: 'check-square', color: '#5c6bf0' };
            case 'PROOF_SUBMITTED': return { name: 'camera', color: '#f59e0b' };
            case 'PAYMENT_REQUEST': return { name: 'money', color: '#ef4444' };
            case 'REQUEST_UPDATE': return { name: 'info-circle', color: '#6366f1' };
    ...
    if (loading) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={styles.loadingText}>ogeysiisyada...</Text>
            </View>
        );
    }
            ...
            >
                {notifications.length === 0 ? (
                    <View style={styles.emptyState}>
                        ...
                        <Text style={styles.emptySub}>Weli wax ogeysiis laguuma soo dirin</Text>
                    </View>
                ) : (
                                        ...
                                        <View style={styles.notificationFooter}>
                                            ...
                                            {notification.link && (
                                                <View style={styles.linkBadge}>
                                                    <Text style={styles.linkText}>Fiiri</Text>
